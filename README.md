...
3차 개발은 Architecture v5의 `A3 Flow Intelligence`를 실행 가능한 코드로 연결합니다. 2차에서 생성한 `demand_routes`를 입력으로, 시뮬레이션 이전에 부하 집중 구간을 저비용으로 사전 탐색합니다.

- edge frequency (edge별 시간당 예상 통과량)
- route concentration (edge별 flow share)
- merge pressure (유입 edge가 둘 이상인 합류 node의 유입 압력)
- station capacity margin·utilization·초과 여부
- 도달 가능한 From-To 수요만 집계하고 미사용 edge는 제외
- `analyze` JSON report에 `edge_flows`·`node_flows`·`station_flows` 섹션 추가
- Simulation Runtime을 바꾸지 않으므로 1차 Golden trace_hash·run_fingerprint 불변
...
정상 예제는 `OD-A-C` 수요에 대해 `E-A-B`·`E-B-C`가 각각 `40/hour`, flow share `0.5`, `ST-A`/`ST-C` capacity margin `80/hour`를 결정론적으로 산출합니다.
