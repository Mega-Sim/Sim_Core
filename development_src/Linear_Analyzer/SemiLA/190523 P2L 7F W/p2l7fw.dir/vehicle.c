...
{
	AMLocationListItem* am_lv1; // 'for each' loop variable
	AMLocationList* am_ls1 = NULL; // 'for each' list
					...
					if (StringCompare(VehGetType(ValidPtr(am_theVehicle, 81, vehicle*)), "DefVehicle") == 0) {
						{
							if (LocGetDistToLoc(ValidPtr(VehGetCurLoc(ValidPtr(am_theVehicle, 81, vehicle*)), 40, simloc*), ValidPtr(am_parkLoc, 40, simloc*)) > ToModelDistance(50000, UNITMILLIMETERS)) {
								...
								{
									am_ls1 = 0;
									ListCopy(LocationList, am_ls1, am2_vParkListFoup);
									for (am_lv1 = (am_ls1) ? (am_ls1)->first : NULL; am_lv1; am_lv1 = am_lv1->next) {
										am2_vParkLocFoup = am_lv1->item;
										{
											{
												if (LocCompare(am2_vParkLocFoup, am_parkLoc) == 0) {
	ListRemoveAllAndFree(LocationList, am_ls1);
													return 0;
												}
											}
										}
									}
									ListRemoveAllAndFree(LocationList, am_ls1); /* End of for each */
								}
								{
