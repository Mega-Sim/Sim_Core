...
public class IBSEMIF implements IUComEventListener, SEMIF {
	...
	/**
	 * m_strSEMFormat - S6F11, S1F17의 포맷을 Daifuku와 Brooks에 맞게 사용
	 */
	...
	// /// Timer 모음
	// javax.swing.Timer S1F13TimerTimer = new javax.swing.Timer(5000,
	// new S1F13TimerTimerListener());
	// javax.swing.Timer ConnectionCheckTimerTimer = new javax.swing.Timer(10000,
	// new ConnectionCheckTimerTimerListener());
	// ///////////////////////////
	// Util Timer
	// ///////////////////////////
	// Timer
	...
	public IBSEMIF(OCSManagerMain ocsMain) {
		m_ocsMain = (OCSManagerMain) ocsMain;
		m_strConfigFileName = ocsMain.m_strIBSEMConfigFileName;
		this.longrunManager = m_ocsMain.getLongRunManager();
	...
	public short SendS2F49ForEx4(MyHashtable pMessage) {
		...
		rsp.SetU2Item(lValue);
		rsp.SetListItem(2); // L 2
		rsp.SetAsciiItem("DELIVERYWAITTIMEOUT");
	...
	public short SendMsg(String strSFMsg) {
		...
		} else if (strSFMsg.equals("S2F33") == true) // Define Report
		{
			...
			int nTotalReportCnt = 30;

			int naReportData[][] = // [9][50] =
			{
					// {ReportID, Num_related VIDs, VIDs......)
					{ 4, 2, 58, 71 }, { 5, 2, 58, 71 }, { 11, 2, 59, 79 }, { 12, 1, 58 }, { 13, 1, 58 }, { 14, 2, 59, 79 }, { 15, 1, 58 }, { 16, 1, 58 }, { 17, 3, 59, 64, 79 }, { 18, 1, 58 },
					{ 19, 1, 58 }, { 20, 1, 58 }, { 21, 1, 58 }, { 22, 2, 70, 69 }, { 23, 3, 70, 68, 54 }, { 24, 3, 70, 68, 54 }, { 25, 2, 70, 58 }, { 26, 2, 70, 69 }, { 27, 3, 70, 68, 54 },
					{ 28, 3, 70, 68, 54 }, { 29, 1, 70 }, { 30, 1, 70 }, { 31, 2, 70, 58 }, { 32, 4, 70, 54, 56, 58 }, { 33, 4, 70, 54, 56, 58 }, { 34, 7, 70, 1, 251, 250, 72, 65, 60 },
					{ 35, 7, 70, 1, 251, 250, 72, 65, 60 }, { 36, 3, 58, 54, 56 }, { 37, 3, 58, 54, 56 }, { 38, 3, 58, 54, 56, 64 }

			/*
			 * 35 70,1,251,250,72,65,60 36 58,54,56 37 58,54,56 38 58,54,56,64
			 */

			};

			// 2013.10.01 by MYM : Report.ini 파일에서 설정하도록 수정
		...
		{
			MyHashtable pMessage = new MyHashtable();
			int nTotalCEIDCnt = 33;
			int naEventLinkData[][] = // new int[36][10] =
			{ // {CEID, Num_related Reports, Reports....)
			{ 503, 1, 34 }, { 504, 1, 35 }, { 51, 1, 4 }, { 52, 1, 5 }, { 101, 1, 11 }, { 102, 1, 12 }, { 103, 1, 13 }, { 104, 1, 14 }, { 105, 1, 15 }, { 106, 1, 16 }, { 107, 1, 17 }, { 108, 1, 18 },
					{ 109, 1, 19 }, { 110, 1, 20 }, { 111, 1, 21 }, { 151, 1, 32 }, { 152, 1, 33 }, { 201, 1, 22 }, { 202, 1, 23 }, { 203, 1, 24 }, { 204, 1, 25 }, { 205, 1, 26 }, { 206, 1, 27 },
					{ 207, 1, 28 }, { 208, 1, 29 }, { 209, 1, 30 }, { 210, 1, 31 }, { 254, 1, 34 }, { 260, 1, 36 }, { 261, 1, 35 }, { 121, 1, 36 }, { 122, 1, 37 }, { 123, 1, 38 }

			};

			// 2013.10.01 by MYM : LinkEventReport.ini 파일에서 설정하도록 수정
		...
		} else if (strSFMsg.equals("S2F35Delete") == true) // Link Event Report
		{
			...
			MyHashtable pMessage = new MyHashtable();
			int nTotalCEIDCnt = 39;

			int naCEIDData[] = // [34] =
			{ 1, 2, 3, 51, 52, 53, 54, 55, 56, 57, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 151, 152, 201, 202, 203, 204, 205, 206, 207, 208, 209, 210, 254, 261, 260, 121, 122, 123 };

			// 2013.10.01 by MYM : EnalbedCEID.ini 파일에서 설정하도록 수정
	...
	public String GetCEventName(long lValue) {
		...
		if (lValue == 1) {
			...
			strReturn = "UnitAlarmCleared";
		} else if (lValue == 504) {
			strReturn = "UnitAlarmSet";
		} else {
	...
	boolean ReceivedMSG(UComMsg msg) {
		...
		if (strSFMsg.equals("S1F1") == true) {
			if (m_strCommState.equals("COMMUNICATING") == true) {
		...
		} else if (strSFMsg.equals("S1F2") == true) {
			m_strControlState = "REMOTE_ONLINE";

			if (m_strSCState.equals("TSC_NONE") == true) {
				m_strSCState = "TSC_INIT";
			}
			SetSEMStatus();
			m_bResponse = true;
		} else if (strSFMsg.equals("S1F4") == true) {
			// 구현할것
			if (m_strSVID.equals("6") == true) {
				if (m_strOnlineSequenceState.equals("INIT") == true) {
					if (m_bAutoInitialize == true)
						SendS1F3(73);
				}
			} else if (m_strSVID.equals("73") == true) {
				// Get SCStatus
			...
			} else if (m_strSVID.equals("91") == true) {
				int nTotalCarrierCount = 0;

				// Get/Set Carrier Data
				//				MyHashtable pReportInfo = new MyHashtable();
				//				nList = msg.GetListItem(); // SVID갯수
				//				nList = msg.GetListItem(); // Carrier갯수
				//				nTotalCarrierCount = nList;
				//				pReportInfo.put("EventName", "RepEnhancedCarrierInfo");
				//				pReportInfo.put("CarrierQty", new Integer(nTotalCarrierCount));
				//				Vector vCarrierID = new Vector();
				//				Vector vVehicleID = new Vector();
				//				Vector vCarrierLocID = new Vector();
				//				Vector vInstallTime = new Vector();
				//				for (int k = 0; k < nTotalCarrierCount; k++) {
				//					nList = msg.GetListItem(); // L,4
				//					BSTRBuff = msg.GetAsciiItem();
				//					vCarrierID.addElement(BSTRBuff);
				//					BSTRBuff = msg.GetAsciiItem();
				//					vVehicleID.addElement(ChangeToDMFormat(BSTRBuff));
				//					BSTRBuff = msg.GetAsciiItem();
				//					vCarrierLocID.addElement(ChangeToDMFormat(BSTRBuff));
				//					BSTRBuff = msg.GetAsciiItem();
				//					vInstallTime.addElement(BSTRBuff);
				//				}
				//
				//				pReportInfo.put("CarrierID", vCarrierID);
				//				pReportInfo.put("VehicleID", vVehicleID);
				//				pReportInfo.put("CarrierLocID", vCarrierLocID);
				//				pReportInfo.put("InstallTime", vInstallTime);
			...
			} else if (m_strSVID.equals("76") == true) {
				//				int nTotalCommandCount = 0;
				//				int nTransferInfoCnt = 0;
				//				// Get/Set Carrier Data
				//				MyHashtable pReportInfo = new MyHashtable();
				//				nList = msg.GetListItem(); // SVID갯수
				//				nList = msg.GetListItem(); // Command갯수
				//				nTotalCommandCount = nList;
				//				pReportInfo.put("EventName", "RepEnhancedTrCmd");
				//				pReportInfo.put("CommandQty", new Integer(nTotalCommandCount));
				//
				//				Vector vMicroTrCmdID = new Vector();
				//				Vector vPriority = new Vector();
				//				Vector vReplace = new Vector();
				//				Vector vTransferState = new Vector();
				//				Vector vCarrierID = new Vector();
				//				Vector vCarrierLocID = new Vector();
				//				Vector vDest = new Vector();
				//
				//				for (int k = 0; k < nTotalCommandCount; k++) {
				//					nList = msg.GetListItem(); // L,3
				//					nList = msg.GetListItem(); // L,3
				//					BSTRBuff = msg.GetAsciiItem();
				//					vMicroTrCmdID.addElement(BSTRBuff);
				//					lValue = msg.GetU2Item();
				//					vPriority.addElement(new Integer(lValue));
				//					lValue = msg.GetU2Item();
				//					vReplace.addElement(new Integer(lValue));
				//					lValue = msg.GetU2Item();
				//					vTransferState.addElement(new Integer(lValue));
				//					nList = msg.GetListItem(); // L,n
				//					nTransferInfoCnt = nList;
				//					for (int j = 0; j < nTransferInfoCnt; j++) {
				//						nList = msg.GetListItem(); // L,3
				//						BSTRBuff = msg.GetAsciiItem();
				//						vCarrierID.addElement(BSTRBuff);
				//						BSTRBuff = msg.GetAsciiItem();
				...
				//				}
				//				pReportInfo.put("MicroTrCmdID", vMicroTrCmdID);
				//				pReportInfo.put("Priority", vPriority);
				//				pReportInfo.put("Replace", vReplace);
				//				pReportInfo.put("TransferState", vTransferState);
				//				pReportInfo.put("CarrierID", vCarrierID);
				//				pReportInfo.put("CarrierLocID", vCarrierLocID);
				//				pReportInfo.put("Dest", vDest);
				...
				if (fileLongRunManager != null) {
					fileLongRunManager.reconcileCarrier(carrerIdSet);
				}
			...
			} else if (m_strSVID.equals("53") == true) {
				int nTotalCarrierCount = 0;
				...
				//				nList = msg.GetListItem(); // Carrier갯수
				//				nTotalCarrierCount = nList;
				//				pReportInfo.put("EventName", "RepVehicleInfo");
				//				pReportInfo.put("VehicleQty", new Integer(nTotalCarrierCount));
				//
				//				Vector vVehicleID = new Vector();
				//				Vector vVehicleState = new Vector();
				//
				//				for (int k = 0; k < nTotalCarrierCount; k++) {
				//					nList = msg.GetListItem(); // L,2
				//					BSTRBuff = msg.GetAsciiItem();
				//					vVehicleID.addElement(BSTRBuff);
				//					lValue = msg.GetU2Item();
				//					vVehicleState.addElement(new Integer(lValue));
				//				}
				//				pReportInfo.put("VehicleID", vVehicleID);
				//				pReportInfo.put("VehicleState", vVehicleState);
			...
			} else if (m_strSVID.equals("114") == true) {
				// Set Flag for OlineSequence
				if (m_strOnlineSequenceState.equals("INIT") == true) {
					if (m_bAutoInitialize == true)
						SendS1F3(118);
				}
			} else if (m_strSVID.equals("118") == true) {
				nList = msg.GetListItem();
				nList = msg.GetListItem();
				int totalBufferCount = nList;
				Set bufferNameSet = new HashSet();

				for (int k = 0; k < totalBufferCount; k++) {
					nList = msg.GetListItem(); // L,2
					BSTRBuff = msg.GetAsciiItem();
					bufferNameSet.add(BSTRBuff);
					lValue = msg.GetU2Item();
				}

				longrunManager.initLongRunManager(bufferNameSet);
			...
			} else if (m_strSVID.equals("4") == true) {
				if (m_strOnlineSequenceState.equals("INIT") == true) {
		...
		// #######################################################
		...
		} else if (strSFMsg.equals("S6F11") == true) {
			MyHashtable pReportInfo = new MyHashtable();
			...
			if (msg.IsEOF() == false) {
				...
				for (int k = 0; k < nReportNo; k++) {
					...
					// 2010.01.15 by MYM : Abort 후 비정상 완료 보고시 DestChange를 보내도록 한다.
					else if (nRPTID == 17) {
						/*
						 * <L 3 <L 3 <A 5 '35213'> <U2 1 '10'> <U2 1 '0'> > <U2
						 * 1 '1'> <L 1 <L 2 <L 3 <A 3 'AAA'> <A 9 'WFHS13_B2'>
						 * <A 9 'WFHS13_B1'> > <A 8 'OHT109_1'> > > >
						 */
						nSubPartNo = msg.GetListItem(); // L,3
						nSubPartNo = msg.GetListItem(); // L,3
						String strTrCmdID = msg.GetAsciiItem();
						int nPriority = msg.GetU2Item();
						lValue = msg.GetU2Item();
						int nResultCode = msg.GetU2Item();
						nSubPartNo = msg.GetListItem(); // L,1
						nSubPartNo = msg.GetListItem(); // L,2
						nSubPartNo = msg.GetListItem(); // L,3
						String strCarrierID = msg.GetAsciiItem();
						String strSourceLoc = msg.GetAsciiItem();
						String strDestLoc = msg.GetAsciiItem();
						String strCarrierLoc = msg.GetAsciiItem();

						if (nResultCode == 1 && strCarrierLoc.equals(strDestLoc) == false && strCarrierLoc.equals(strSourceLoc) == false) {
							destChange(strTrCmdID, strCarrierID, strCarrierLoc, nPriority);
						} else if (nResultCode == 0 && "TransferCompleted".equals(strEventName) == true) {
							FileLongRunManager fileLongRunManager = m_ocsMain.getFileLongRunManager();
							if (fileLongRunManager != null) {
								fileLongRunManager.transferCompleted(strCarrierID, strSourceLoc, strDestLoc);
							}
							longrunManager.transferCompleted(strTrCmdID);
						}
					}
					// 2012.01.25 by LWG [Cancel/Abort 내리기] : transfering
					else if (nRPTID == 21) {
						nSubPartNo = msg.GetListItem(); // L,1
						String trcmdId = msg.GetAsciiItem();
						longrunManager.setTrcmdTransferring(trcmdId);
					}
					// 2012.01.25 by LWG [Cancel/Abort 내리기] : transfer abort complete
					else if (nRPTID == 11) {
						nSubPartNo = msg.GetListItem(); // L,2
						nSubPartNo = msg.GetListItem(); // L,3
						String strTrCmdID = msg.GetAsciiItem();
 ...
 * 
 */
	private void destChange(String trcmdId, String carrierId, String sourceLoc, int priority) {
		MyHashtable htCommandInfo = new MyHashtable();
		htCommandInfo.put("TSC", "TOCS411");
		htCommandInfo.put("MicroTrCmdID", trcmdId);
		htCommandInfo.put("MicroTrCmdType", "TRANSFER");
		htCommandInfo.put("CarrierID", carrierId);
		htCommandInfo.put("CarrierLocID", "");
		htCommandInfo.put("Source", sourceLoc);
		String newDest = longrunManager.getRandomBufferPort();
		htCommandInfo.put("Dest", newDest);
		htCommandInfo.put("Priority", new Integer(priority));
	...
	public boolean loadLinkEventReportConfig(MyHashtable pMessage) {
		StringBuffer FilePathName = new StringBuffer();
		String strFileName = "LinkEventReport.ini";
	...
	public boolean loadEnabledCEIDConfig(MyHashtable pMessage) {
		StringBuffer FilePathName = new StringBuffer();
		String strFileName = "EnabledCEID.ini";
	...
	@Override
	public int sendS2F41(String cmd, String carrierId, String carrierLocId) {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public int sendS2F49(String cmd, boolean isValid) {
		// TODO Auto-generated method stub
		return 0;
	}
