...
/**
 * <p>Title: SAC</p>
 * <p>Description: 온양 물류센터 자동창고 시스템</p>
 * <p>Copyright: Copyright (c) 2004</p>
 * <p>Company: 삼성전자 메카트로닉스센터 지능시스템연구소</p>
 * @author 이수정 수석, 정정주 책임, 임광영 선임
 * @version 1.0
 */

class MsgVector extends Vector {
		...
		while (idxS < size) {
			...
			if (oneS.compareTo("%") == 0) { // 특수 문자인 경우
				...
				if (oneS.compareTo("d") == 0) { // int 타입(decimal)
					Integer nValue = new Integer(Integer.parseInt(strData.substring(idxD, idxD + scanlength).trim()));
					super.add(nValue);
				} else if (oneS.compareTo("f") == 0) { // float 타입
					Double dValue = new Double(Double.parseDouble(strData.substring(idxD, idxD + scanlength).trim()));
					super.add(dValue);
				} else if (oneS.compareTo("s") == 0) { // string 타입
					String sValue = new String(strData.substring(idxD, idxD + scanlength)).trim();
					super.add(sValue);
				} else if (oneS.compareTo("b") == 0) { // Boolean 타입
					String strBoolean = strData.substring(idxD, idxD + scanlength).trim().toUpperCase();
					Boolean bValue;
			...
			if (oneS.compareTo("%") == 0) {
				...
				if (oneS.compareTo(".") == 0) {
					...
					if (strPrecision.length() > 0) {
						...
						if ((printlength > 0) && (nPrecision + 2) > printlength) { // 0.xxxx --> 4+2=6자리임
							System.out.println("ERROR: (nPrecision value(" + nPrecision + ") must be 2 more than printlength value(" + printlength + ").\nINPUTTED FORMAT: " + strFormat
									+ "\nCURRENT RESULT: " + S);
							return "";
						}
					}
				...
				if (oneS.compareTo("d") == 0) { // int 타입(decimal)
					...
					if (printlength > 0 && tempStr.length() > printlength) {
						System.out.println("ERROR: Integer(" + tempStr + ")'s number length is longer than assigned length(" + printlength + ").\nINPUTTED FORMAT: " + strFormat + "\nCURRENT RESULT: "
								+ S);
						return "";
					}
				} else if (oneS.compareTo("f") == 0) { // float 타입
					...
					if ((printlength > 0) && (nIntLength > printlength)) {
						System.out.println("ERROR: Float(" + tempStr + ")'s integer part length is longer than assigned length(" + printlength + ").\nINPUTTED FORMAT: " + strFormat
								+ "\nCURRENT RESULT: " + S);
						return "";
					}
					...
					if (nPrecision > -1) { // precision이 정의되어 있다면,
						if (nPrecision == 0) { // precision = 0이라면 정수부분만 출력
							...
							tempStr = sNumberPart[0] + "." + sNumberPart[1].substring(0, nPrecision);
						}
					} else {
					...
					// 잘못된 스트링이 있을때.
					System.out.println("ERROR: cannot convert inputted string by using strFormat, invalid character: " + oneS + "\nINPUTTED FORMAT: " + strFormat + "\nCURRENT RESULT: " + S);
					return "";
				}
	...
	public int toInt(String key, int index) {
		...
		if (o != null) {
			...
			if (sClassName.indexOf("Vector") > -1) {
				...
				if (v.elementAt(index).getClass().getName().indexOf("String") > -1) {
					s = (String) v.elementAt(index);
				} else if (v.elementAt(index).getClass().getName().indexOf("Integer") > -1) {
					s = v.elementAt(index).toString();
				} else {
					s = null;
				}
...
class CMessage {
	...
	/* Get Message Item */
	boolean GetMessageItem(String Name, MsgVector ValueList, int Index, boolean ErrorLog) {
		boolean bReturn;
	...
	void ErrorLog(String Command, String Name) {
		...
		try {
			...
			BufferedWriter out = new BufferedWriter(new FileWriter(FilePathName.toString(), true));
			out.newLine();
...
class utilLog {
	...
	//--------------------------------------------------------------------------
	// make log string
	synchronized void WriteReturnLog(String filename, String log1, String log2, boolean bHourly) {
	...
	synchronized void DeleteAutoLogFiles(int DaysBefore) {
		...
		if (children != null) {
			for (int i = 0; i < children.length; i++) {
				...
				difftime = (System.currentTimeMillis() - updatedtime) / 1000 / 24 / 3600;
	...
	synchronized double getCBuilderDay() {
		double dTime;
		dTime = (double) (System.currentTimeMillis() / 3600000.0 / 24.0 + 25569.374994213);
		return dTime;
	}
...
class CLogManager {
	...
	// Time + Log
	synchronized public boolean WriteReturnTimeLog(String logfilename, String logstring1, String logstring2, boolean bHourly) {
		int Pos;
		String Name1;
		String Text1;
	...
	// Log
	synchronized public boolean WriteReturnLog(String logfilename, String logstring1, String logstring2, boolean bHourly) {
		int Pos;
		String Name1;
		String Text1;
		Date currTime = new Date();
