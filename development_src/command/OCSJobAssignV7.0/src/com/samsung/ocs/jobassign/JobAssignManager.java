...
/**
 * JobAssignManager Class, OCS 3.0 for Unified FAB
 * 
...
public class JobAssignManager extends AbstractOcsThread {
	private JobAssign jobAssign;
	...
	// 2011.11.04 by PMM
	private static final String JOBASSIGN = "JobAssign";
	...
	private static final String INITIALIZING_STATIONMANAGER = "Initializing StationManager ..."; // 2015.04.14 by KYK
	...
	/**
	 * Constructor of JobAssignManager class.
	 */
	public JobAssignManager() {
		initializeManager();
		jobAssign = new JobAssign();
		new LogManager(OcsConstant.JOBASSIGN);
	}
	...
	private void initializeManager() {
		...
		ocsInfoManager = OCSInfoManager.getInstance(OCSInfo.class, dbAccessManager2, true, true, 500);
		carrierLocManager = CarrierLocManager.getInstance(CarrierLoc.class, dbAccessManager1, true, true, 500);
		hidManager = HIDManager.getInstance(Hid.class, dbAccessManager3, true, true, 300);
		AreaManager.getInstance(Area.class, dbAccessManager1, true, true, 1100);
		nodeManager = NodeManager.getInstance(Node.class, new DBAccessManager(), true, true, 500);
		...
		collisionManager = CollisionManager.getInstance(Collision.class, dbAccessManager3, true, false, 500);
		zoneControlManager = ZoneControlManager.getInstance(ZoneControl.class, dbAccessManager1, true, false, 500);
		VehicleManager.getInstance(Vehicle.class, new DBAccessManager(), true, false, 500);
		alarmManager = AlarmManager.getInstance(Alarm.class, dbAccessManager2, true, true, 300);
		LocalGroupInfoManager.getInstance(LocalGroupInfo.class, new DBAccessManager(), true, true, 700);
		TrCmdManager.getInstance(TrCmd.class, new DBAccessManager(), true, false, 200);
		eventHistoryManager = EventHistoryManager.getInstance(EventHistory.class, dbAccessManager3, false, true, 500);
		...
		// 2013.05.16 by KYK : ParkManager 보다 먼저 생성되어야 함
		stationManager = StationManager.getInstance(Station.class, dbAccessManager1, true, true, 500);
		parkManager = ParkManager.getInstance(Park.class, dbAccessManager1, true, true, 300);
		// 2020.06.22 by YSJ : 이적재 위치의 ParkNode 구분
		parkManager.updateParkPortCount();
		
		// 2013.07.10 by KYK
		materialControlManager = MaterialControlManager.getInstance(MaterialControl.class, dbAccessManager1, true, false, 0);
	}
	...
	protected void initialize() {
		// 2013.06.25 by MYM : 100 -> 1000로 변경
		interval = 1000;
	...
	private void manageServiceState() {
		if (this.requestedSerivceState == MODULE_STATE.REQOUTOFSERVICE) {
			...
			jobAssign.traceJobAssignMain("Activated!");
		}
		
		if (this.serviceState == MODULE_STATE.OUTOFSERVICE) {
			if (++outOfServiceCheckCount >= 5) {
				jobAssign.traceJobAssignMain("      [OUTOFSERVICE]");
				outOfServiceCheckCount = 0;
			}
		}
	}
	...
	private void manageRuntimeUpdate() {
		...
		// 2011.11.04 by PMM
		if (ocsInfoManager.getJobAssignUpdate() != RUNTIME_UPDATE.NO) {
			if (this.serviceState == MODULE_STATE.INSERVICE) {
				if (ocsInfoManager.getJobAssignUpdate() == RUNTIME_UPDATE.YES) {
//					registerAlarmText(RUNTIMEUPDATE_START_INSERVICE);
					...
					registerEventHistory(new EventHistory(EVENTHISTORY_NAME.RUNTIME_UPDATE, EVENTHISTORY_TYPE.SYSTEM, "", RUNTIMEUPDATE_START_INSERVICE, 
							"", "", EVENTHISTORY_REMOTEID.JOBASSIGN, "", EVENTHISTORY_REASON.RUNTIME_UPDATE), false);
					...
					ocsInfoManager.setJobAssignUpdate(RUNTIME_UPDATE.DONE);
				} else if (ocsInfoManager.getJobAssignUpdate() != RUNTIME_UPDATE.DONE) {
					ocsInfoManager.setJobAssignUpdate(RUNTIME_UPDATE.NO);
				}
			} else if (this.serviceState == MODULE_STATE.OUTOFSERVICE) {
				if (ocsInfoManager.getJobAssignUpdate() == RUNTIME_UPDATE.DONE) {
//					registerAlarmText(RUNTIMEUPDATE_START_OUTOFSERVICE);
	...
	private void updateManager() {
		...
		assert materialControlManager != null;
...
//			registerAlarmText(INITIALIZING_ZONECONTROLMANAGER);
			...
			// 2015.04.14 by KYK
			registerAlarmText(INITIALIZING_STATIONMANAGER);
			...
			jobAssign.initializeForRuntimeUpdate();
			
		} catch (Exception e) {
			traceRuntimeUpdate(RUNTIMEUPDATE_FAILED);
			jobAssign.traceJobAssignException("updateManager()", e);
		}
	}
	
	// 2011.11.04 by PMM
	private void registerAlarmText(String alarmText) {
		alarmManager.registerAlarmText(JOBASSIGN, alarmText);
	}
	...
	private void registerAlarmTextWithLevel(String alarmText, ALARMLEVEL alarmLevel) {
		if (alarmManager != null) {
			...
			alarmManager.registerAlarmTextWithLevel(JOBASSIGN, alarmText, alarmLevel.toConstString());
		}
	}
	
	private void unregisterAlarmText(String alarmText) {
		alarmManager.unregisterAlarmText(JOBASSIGN, alarmText);
	}
	...
	public void traceJobAssignMain(String message) {
		jobAssign.traceJobAssignMain(message);
	}
}
