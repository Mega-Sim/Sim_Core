...
public class LogManager extends AbstractOcsThread {
	...
	/**
	 * Constructor of LogManager class.
	 */
	public LogManager(String module) {
		...
		exceptionLog = Logger.getLogger(module + OcsConstant.EXCEPTION);
	...
	protected void mainProcessing() {
		...
		if (calendar.get(Calendar.MINUTE) == 0 && calendar.get(Calendar.SECOND) == 0) {
			calendar.add(Calendar.DATE, (-1) * (ocsInfoManager.getLogHoldingPeriod(module)));
			String timeBefore = formatter.format(calendar.getTime());
			deleteLog(logPath, ocsInfoManager.getLogHoldingPeriod(module), timeBefore);
		}
	}
