...
/**
 * Read Class, OCS 3.0 for Unified FAB
 * 
 * NormalComm¿« read result
 * 
...
public class Read2 extends Read {

	/**
	 * Constructor of Read2 class.
	 * 
	 * @param recieveMessage
	 */
	public Read2(byte[] recieveMessage) {
		super(recieveMessage);
	}
	
	public byte[] getFoupIdDataBytes() {
		byte[] b = new byte[64];
		System.arraycopy(recieveMessage, 337, b, 0, 64);
		return b;
	}
	...
	public String getKey() {
		return String.format("%s[%s:%d]", getMachineId(), NORMAL_COMMTYPE.READ, getStbNumber() );
	}
	...
	public String toLogString(String sourceIpAddress) {
		...
		sb.append(" STBNO [").append(getStbNumber()).append("] CARRIERDETECT[").append(getCarrierDetectByte()).append("] READRESULT[").append(getReadResultByte()).append("] IDDATA[").append(getIdData()).append("] FOUPIDDATA[").append(getFoupIdData()).append("]");
		sb.append("\n");
