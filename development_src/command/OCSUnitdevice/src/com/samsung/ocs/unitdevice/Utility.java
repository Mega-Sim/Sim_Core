			...
			if (oneS.compareTo("%") == 0)
			{
				...
				if (oneS.compareTo(".") == 0)
				{
					...
					if (strPrecision.length() > 0)
					{
						...
						if ( (printlength > 0) && (nPrecision + 2) > printlength)
						{ // 0.xxxx --> 4+2=6자리임
				...
				if (printlength > 0)
				{
					if (tempStr.length() > printlength)
					{ // 문자열이 더 길게 생성되었다면, 남는 길이만큼 잘라낸다.
...
class utilLog
{
	...
	//  2007.06.27 Display Exception log (by PTE)
			...
			boolean bHourly)
	{
		...
		if (k >= m_pFileList.size() || pLogItem == null)
		{
	...
	synchronized void OutputLog(String filePathName, String log)
	{
		...
		try
		{
			...
			out = new BufferedWriter(new FileWriter(filePathName, true));
			//out.newLine();
			out.write(log);
			out.newLine();
	...
	//2006.07.03 by N.Y.K
	synchronized void OutputLog(String filePathName,FileWriter fwFile ,String log)
	{
		// 2011.01.07 by PMM
		BufferedWriter out = null;
...
class CLogManager
{
	...
	synchronized public boolean SaveLogManagerErrLog(String sErrLog)
	{
		...
		try
		{
			...
			sysout = new FileWriter(lmLogFile, true);
