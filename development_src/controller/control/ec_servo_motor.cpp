...
#define printbin(...)
    ...
    EcServoMotor::EcServoMotor()
            : ServoMotor(), msg_recvq_(nullptr), shm_(nullptr), out_pdo_(nullptr), in_pdo_(nullptr),
              initialized_(false), enabled_(false), emulation_mode_(ECAT_EMULATION_MODE), enc_offset_(0)
    {
        servo_vendor = 0;
        servo_type_ = 0;
    }
    ...
    bool EcServoMotor::initialize(int axis, ServoOpMode mode, const char* model,  bool enable_on_init)
    {
        ...
        if (!emulation_mode_) {
            ...
            bus_index = res.bus_index;
            servo_type_ = res.slave_type;
            servo_vendor = res.servo_vendor;
            module_num = res.servo_module_num;
            
            printf("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@[axis_%d] vendor=%d\t type:%d\t bus_index:%d\t module_num:%d\n",axis, servo_vendor, servo_type_,bus_index,module_num);
...
//            SEP_REQUIRE_RET(coeSdoDownload(COE_STORE_PARAM, 3, (uint8_t * ) & dat, sizeof(dat)), false);
    ...
    bool EcServoMotor::setMaxTrq(uint16_t trq)
    {
        ...
        if (!emulation_mode_) {
            if(servo_vendor == ecvendor_yaskawa) {
                SEP_LOGT("Yaskawa setMaxTrq is called!");
                trqlimit_p = 0x2402;
                trqlimit_n = 0x2403;
            } else if(servo_vendor == ecvendor_rsa) {
    ...
    bool EcServoMotor::waitForStateReady(int state1) 
   {
    ...
    bool EcServoMotor::adjustingCommand()
    {/*
    	int nCount = 0;
        int32_t user_param = 1;
        ...
        SEP_REQUIRE_RET(setServoMode(ServoOpMode::HOME), false);
      
		SEP_REQUIRE_RET(enable(), false);

        controlWord().setBit(4, true);
                
        const int MAX_LOOP = 1000;  // 대략 500 * 1ms = 0.5s
        bool homing_ok = false;

        for (int i = 0; i < MAX_LOOP; ++i) {
            usleep(bus_cycle_us_);   // 1ms 주기와 맞춰서 대기
            SEP_LOGT("Waiting count: %d (10):%d (12):%d",i, statusWord().getBit(10),statusWord().getBit(12));
            if (statusWord().fault) {
                homing_ok = false;
                break;
            }
            if (statusWord().getBit(10) && statusWord().getBit(12)) {
                homing_ok = true;
                break;
            }
        }

        controlWord().setBit(4, false);
    ...
    bool EcServoMotor::adjustingCommandRSA()
    {
        ...
        int32_t user_param = 1;
        int sdo_index = 0;
        if(servo_type_ == SINGLE_SERVO)	        sdo_index = 0x3001; // 단축
        else if(servo_type_ == MULTI_SERVO)     sdo_index = 0x2C1C; // 다축(+3C1C)
...
//        if (enabled) disable();
    ...
    bool EcServoMotor::resetEncoder(bool soft)
    {
        int32_t loopCnt = 0;
        clock_t start, end;
        start = clock();
...
//        if (enabled) disable();
        ...
        if (soft) { // Make current pulse to be read zero
            ...
            if(servo_vendor == ecvendor_yaskawa) {
                ...
                SEP_REQUIRE_RET(coeSdoUpload(0x27E4, 0, (uint8_t *) &home_offset_pulse, sizeof(home_offset_pulse)) > 0, false);
                ...
                SEP_REQUIRE_RET(coeSdoDownload(0x27E4, 0, (uint8_t *) &new_home_offset_pulse, sizeof(new_home_offset_pulse)), false);
                SEP_REQUIRE_RET(coeSdoDownload(COE_USER_PARAM_CONFIG, 0, (uint8_t *) &user_param, sizeof(user_param)), false);
    ...
    bool EcServoMotor::setReferenceCSP(double p, double* voffs, double* toffs) //  CSP{p, voff, toff}
    {
        ...
        if(!initialized_ || op_mode_ != ServoOpMode::CSP)
			return false;
        SEP_REQUIRE_RET(statusWord().fault==0, false);
        ref_pos_ = unit2pos(p);
        if (emulation_mode_) {
    ...
    bool EcServoMotor::setMasterAxis(int slave_id)
    {
        ...
        return true;  // SetMasterAxis 기능 제거
