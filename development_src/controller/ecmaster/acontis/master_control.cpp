...
namespace sephi {
    ...
    bool EcMasterSvc::getSlaveInfos() {
        ...
        for (wAutoIncAddress = 0; wAutoIncAddress != 1; wAutoIncAddress--) {
            ...
            if (slaveinfo.vendor_id == ecvendor_yaskawa && slaveinfo.product_code == 0x02200901 /* sigma 7*/) {
                slaveinfo.type = SlaveType::CIA402_COMPLIANT_SERVO_SINGLE;
                axis_count = 1;
                SEP_LOGT("Yaskawa Servo Detected\n");
            } else if (slaveinfo.vendor_id == ecvendor_yaskawa && slaveinfo.product_code == 0x02200902 /* sigma X*/) {
                slaveinfo.type = SlaveType::CIA402_COMPLIANT_SERVO_MULTI;
                axis_count = 2;
                SEP_LOGT("Yaskawa Dual-Servo Detected\n");
                ...
                SEP_LOGT("RSA Triple-Servo Detected\n");
                       ...
                       (slaveinfo.product_code == 0x02020033) /* Quadruple axis servo pack*/) {
                slaveinfo.type = SlaveType::CIA402_COMPLIANT_SERVO_MULTI;
                axis_count = 4;
                SEP_LOGT("RSA Quadruple-Servo Detected\n");
            } 
            ...
            if(axis_count > 1)
            {
                ...
                slaveinfo.in_pd_length = oCfgSlaveInfo.dwPdSizeIn / 8 / axis_count;
                slaveinfo.out_pd_length = oCfgSlaveInfo.dwPdSizeOut / 8 / axis_count;
    ...
    bool EcMasterSvc::setupSvc() // PREOP-> SAFEOP
    {
        ...
        for (auto &slave : slaves_) {
            ...
            // context 설정
            if (slave.type == SlaveType::CIA402_COMPLIANT_SERVO_SINGLE
            || slave.type == SlaveType::CIA402_COMPLIANT_SERVO_MULTI) {
