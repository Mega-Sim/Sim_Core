...
namespace sephi
{
    ...
    class EcMasterRpcImpl final : public EcMasterRpc::Service
    {
        ...
        Status requestSlave(ServerContext *context, const Request *request, Response *reply) override
        {
            ...
            reply->set_bus_index(res.res.bus_index);
            reply->set_servo_module_num(res.res.servo_module_num);
            reply->set_servo_vendor(res.res.servo_vendor);
            reply->set_slave_type(res.res.slave_type);
            return Status::OK;
        }
