...
namespace sephi {
        ...
        , stocker_io_(nullptr), ecat_state_(0), out_pdo(nullptr), in_pdo(nullptr)
    {
        ...
        shm_ = std::make_unique<SepShm>(EM_SHM_KEY(), EM_SHM_SIZE);
...
//        if (ecat_state_ != eEcatState_OP) return; 20181226
        ...
        for(const auto& slave: slaves_){
            ...
            if (slave.type == CIA402_COMPLIANT_SERVO_SINGLE
                || slave.type == CIA402_COMPLIANT_SERVO_MULTI
                ...
                // fault check & reset
                if (slave.type == CIA402_COMPLIANT_SERVO_SINGLE
                    || slave.type == CIA402_COMPLIANT_SERVO_MULTI) {    // yjchoe
