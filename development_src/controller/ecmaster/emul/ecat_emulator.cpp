...
namespace sephi
{
    ...
    bool ECatEmulator::loadENI(const string& eni_file_name)
    {
        ...
        try {
            ...
            for( auto i = 0; i < slaves_eni.size(); i++) {
                ...
                switch(slavetype) {
                    case SlaveType::CIA402_COMPLIANT_SERVO_SINGLE:
                    case SlaveType::CIA402_COMPLIANT_SERVO_MULTI:
    ...
    bool ECatEmulator::updateMasterInfo(ECatSlaveInfo& _slave)
    {
        ...
        // context 설정
        if (slave.type == SlaveType::CIA402_COMPLIANT_SERVO_SINGLE || slave.type == SlaveType::CIA402_COMPLIANT_SERVO_MULTI) {  // yjchoe
