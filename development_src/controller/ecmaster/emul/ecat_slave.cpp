...
namespace sephi {
    ...
    {
        if (type_ == SlaveType::CIA402_COMPLIANT_SERVO_SINGLE) return "Singe-axis Servo";
        else if (type_ == SlaveType::CIA402_COMPLIANT_SERVO_MULTI) return "Multi-axis Servo"; 
        else if (type_ == SlaveType::FINE_STK_IO) return "FINE_STK_IO";
        else if (type_ == SlaveType::FINE_OHT_FRONT_IO) return "FINE_OHT_FRONT_IO";
        else if (type_ == SlaveType::FINE_OHT_REAR_IO) return "FINE_OHT_REAR_IO";
        return "Unknown";
    }
    ...
    int32_t ECatSlaveModel::getSlaveType(uint32_t vendor_id, uint32_t product_code)
    {
        ...
        if (vendor_id == ecvendor_yaskawa) {
            type = SlaveType::CIA402_COMPLIANT_SERVO_SINGLE;
        } else if (vendor_id == ecvendor_rsa) {
            type = SlaveType::CIA402_COMPLIANT_SERVO_SINGLE;  // yjchoe
